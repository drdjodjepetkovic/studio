...
interface TeamMemberDjordjeSectionProps extends SectionProps {
  isLastSection?: boolean;
  onNavigateNext?: () => void;
  isPageScrolling?: boolean;
}

export const TeamMemberDjordjeSection: React.FC<TeamMemberDjordjeSectionProps> = ({ 
  id = "team-member-djordje", 
  isLastSection, 
  onNavigateNext, 
  isPageScrolling 
}) => {
  const member = teamMembers[1]; // Dr. Đorđe Petković
  const content = sectionText.teamMemberDjordje;

  if (!member) return null; // Should not happen
