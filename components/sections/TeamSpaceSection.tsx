...
interface TeamSpaceSectionProps extends SectionProps {
  isLastSection?: boolean;
  onNavigateNext?: () => void;
  isPageScrolling?: boolean; // New prop
}

export const TeamSpaceSection: React.FC<TeamSpaceSectionProps> = ({ id = "team-space", isLastSection, onNavigateNext, isPageScrolling }) => {
  return (
    <SectionWrapper 
      id={id} 
      className=""
    ...
    >
      ...
      >
        {sectionText.teamMemberSlobodanka.headline} {/* Placeholder, content will be split */}
      </AnimatedText>
      <AnimatedText 
        as="p" 
        className="text-lg md:text-xl text-palm-gray-600 mb-12 max-w-2xl mx-auto leading-relaxed" 
        delay="delay-200"
        isPageScrolling={isPageScrolling}
      >
        {sectionText.teamMemberSlobodanka.subhead} {/* Placeholder */}
      </AnimatedText>
      ...
      <div className="space-y-16 max-w-4xl mx-auto">
        {teamMembers.map((member, index) => (
          <AnimatedText 
            key={member.id} 
            as="article" 
            className="grid md:grid-cols-12 gap-6 md:gap-8 items-start bg-palm-white/60 backdrop-blur-md p-6 md:p-8 rounded-lg shadow-xl overflow-hidden"
      ...
      >
        <Button href="#new-contact" variant="primary" size="md" arrow>
          {sectionText.teamMemberDjordje.cta || "Zakažite Termin"} {/* Placeholder for CTA */}
        </Button>
      </AnimatedText>
    </SectionWrapper>
  );
